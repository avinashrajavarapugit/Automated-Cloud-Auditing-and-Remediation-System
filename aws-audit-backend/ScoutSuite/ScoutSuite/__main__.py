...
async def _run(provider,
    ...
    # else:
    #     return 0
    # Instead of opening the report in the browser, read the HTML content
    try:
        with open(html_report_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
    except Exception as e:
        print_exception('Failure while reading HTML report: {}'.format(e))
        return 110

    # Return the HTML content so that it can be displayed on the frontend.
    # Optionally, you can wrap it in a JSON object, e.g.:
    return {"report": html_content, "errors": bool(ERRORS_LIST)}
